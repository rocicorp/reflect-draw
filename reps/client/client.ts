...
import { PingMessage } from "../src/protocol/ping";
import {
  Replicache,
  MutatorDefs,
  Poke,
...
import { NullableVersion, nullableVersionSchema } from "../src/types/version";
import { downstreamSchema } from "../src/protocol/down";
import { PokeBody } from "../src/protocol/poke";
import { PushBody, PushMessage } from "../src/protocol/push";
import { GapTracker } from "../src/util/gap-tracker";
...
async function getBaseCookie(rep: Replicache) {
  ...
  rep.puller = async (req): Promise<PullerResult> => {
    const val = (await req.json()) as PullRequest;
