...
export async function withExecutor<R>(
  ...
  const executor = async (sql: string, params?: any[]) => {
    try {
      lc.debug?.(`Executing: ${sql}, ${params}`);
      return await client.query(sql, params);
    } catch (e) {
      lc.error?.(`Error executing: ${sql}, ${params}`, e);
      throw new Error(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function transact<R>(lc: LogContext, body: TransactionBodyFn<R>) {
  return await withExecutor(lc, async (executor) => {
...
async function transactWithExecutor<R>(
  ...
  lc.error?.("Tried to execute transacation too many times. Giving up.");
